...
if has openclaw; then
  ...
  openclaw config set agents.defaults.model.fallbacks '["openai/gpt-5.3-codex","openai/gpt-5.4"]' --json 2>/dev/null || true
...
echo "    5b. (Optional) Scale Discord lanes/parallel threads: bash .claude/bootstrap/openclaw_discord_scale_setup.sh"
...
echo ""
echo "  Browser:"
echo "    - Default: OpenClaw managed browser profile (openclaw)"
echo "    - Docs: https://docs.openclaw.ai/tools/browser"
echo "    - Docker Selenium browser is optional (set OPENCLAW_DOCKER_BROWSER=1)"
echo ""
