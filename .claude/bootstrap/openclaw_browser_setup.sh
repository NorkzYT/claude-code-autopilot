...
generate_env_file() {
  ...
  local chrome_args="--remote-debugging-address=0.0.0.0 --remote-debugging-port=${CDP_PORT} --no-first-run --disable-features=Translate --start-maximized --window-size=1920,1080"
  if [[ -n "$ext_flags" ]]; then
...
CHROME_ARGS="--remote-debugging-address=0.0.0.0 --remote-debugging-port=${CDP_PORT} --no-first-run --disable-features=Translate --start-maximized --window-size=1920,1080"
if [[ -n "$EXT_FLAGS" ]]; then
  CHROME_ARGS="$CHROME_ARGS $EXT_FLAGS"
fi
