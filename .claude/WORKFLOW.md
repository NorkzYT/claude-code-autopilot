...
2. **Build**   -- Run BUILD_CMD from TOOLS.md (via `openclaw-local-workflow.sh`)
3. **Test**    -- Run TEST_CMD from TOOLS.md
4. **Confirm** -- Run CONFIRM_CMD (smoke check) from TOOLS.md
5. **Deploy**  -- Commit, push to feature branch, monitor CI/CD
6. **Verify**  -- Check CI passes, read failure logs if not (`gh run view --log-failed`)
7. **Next**    -- Close issue, show next bugs (via `issue-lifecycle` skill)

### How it works

- **TOOLS.md**: Auto-generated by `analyze_repo.sh`, contains detected BUILD_CMD, TEST_CMD, RUN_CMD, CONFIRM_CMD
- **openclaw-local-workflow.sh**: Reads TOOLS.md and runs the full build -> run -> test -> confirm pipeline
- **workflow-report.local.json**: Output of the workflow script; closer reads this to verify lifecycle
- **Fallback**: If TOOLS.md doesn't exist, autopilot runs `analyze_repo.sh` first. If that fails, falls back to manual command detection.
...
| `.claude/bootstrap/analyze_repo.sh` | Detects BUILD/TEST/RUN/CONFIRM commands -> TOOLS.md |
| `workflow-report.local.json` | Output of workflow script (step results) |
| `.claude/skills/issue-lifecycle/SKILL.md` | Close issue + list next bugs |
