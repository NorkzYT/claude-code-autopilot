...
if [[ "$INSTALL_OPENCLAW" == "1" ]]; then
  ...
  echo "    openclaw dashboard   # shows local URL and remote access hints"
  echo ""
  echo "  Remote dashboard (Tailscale):"
  echo "    OpenClaw setup auto-configures Tailscale Serve (HTTPS) when Tailscale is installed."
  echo "    Prefer the HTTPS Tailscale Serve URL over http://100.x.x.x:18789"
  echo "    If Serve permissions are denied, run once: sudo tailscale set --operator=\$USER"
  echo "    First secure UI connect may require: openclaw devices approve <requestId>"
  echo ""
